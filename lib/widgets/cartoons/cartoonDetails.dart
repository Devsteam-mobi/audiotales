...
class CartoonDetails extends StatefulWidget{
  ...
  CartoonDetails({Key key, this.taleInf, this.cartoonDetailsJSON}) : super(key: key);
...
class _CartoonDetailsState extends State<CartoonDetails>{
  ...
  Future<List<CartoonDetailsJSON>> cartoonDetailsAPI() async{
    var id = widget.taleInf.id;
    var response = await http.get('URL');


    if(response.statusCode == 200){
      List cartoonDetailsURL = json.decode(utf8.decode(response.bodyBytes));
      return cartoonDetailsURL
          .map((cartoonDetailsURL) => new CartoonDetailsJSON.fromJson(cartoonDetailsURL))
          .toList();
    } else
