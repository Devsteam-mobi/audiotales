...
class MusicPlayerWidget extends StatefulWidget {
  ...
  MusicPlayerWidget({this.url, this.autoPlay, this.musicDetailsJSON});

  @override
  State<StatefulWidget> createState() {
    return _MusicPlayerWidgetState(url, autoPlay);
  }
}
...
class _MusicPlayerWidgetState extends State<MusicPlayerWidget> {
  ...
  _MusicPlayerWidgetState(this.url, this.autoPlay);
  ...
  void checkFav() {
    if (favoriteDataMusic.length > 0 && checkFavor == false) {
      for (int i = 0; i < favoriteDataMusic.length; i++) {
        if (favoriteDataMusic[i].name == widget.musicDetailsJSON.name) {
          checkFavor = true;
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    void writeToFile() {
      ...
      if (fileExists && checkFavor == false) {
        favoriteDataMusic.add(f);
        jsonFile.writeAsStringSync(json.encode(favoriteDataMusic));
      } else if(checkFavor == true){
        favoriteDataMusic.removeWhere((item) => item.id == audioId);
        jsonFile.writeAsStringSync(json.encode(favoriteDataMusic));
      }
      favoriteDataMusic = json.decode(jsonFile.readAsStringSync());
      favoriteDataMusic = favoriteDataMusic.map((fav) => new MusicDetailsJSON.fromJson(fav)).toList();
            ...
            Align(
              ...
              child: Text(
                _position != null
                    ...
                    : '${widget.musicDetailsJSON.duration}',
                style: TextStyle(fontSize: 12.0,),
              ),
            ),
          ],),
                ...
                MaterialButton(
                  ...
                  child: Container(
                    ...
                    child: SizedBox(
                      ...
                      child: Align(
                        alignment: Alignment.center,
                        child: Text((checkFavor == false || favoriteDataMusic == null) ? "Добавить в любимое" : "Удалить из любимого",
                  ...
                  onPressed:() => setState(() => {
                    ...
                    } else if (checkFavor == false || favoriteDataMusic == null) {
                      f.favorite = true,
                      writeToFile(),
                      checkFavor = true,
                      Fluttertoast.showToast(
    ...
    _audioPlayer.onDurationChanged.listen((duration) {
      ...
      if (Theme.of(context).platform == TargetPlatform.iOS) {
        ...
        _audioPlayer.setNotification(
            title: widget.musicDetailsJSON.name,
            imageUrl: 'URL',
